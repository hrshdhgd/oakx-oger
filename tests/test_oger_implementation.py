...
class TestOGERImplementation(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.input_file = (
            Path(__file__).resolve().parents[1] / "tests/input/text.txt"
        )
        self.impl.output_dir = (
            Path(__file__).resolve().parents[1] / "tests/output/"
        )
        self.impl.terms_dir = (
            Path(__file__).resolve().parents[1] / "tests/terms/"
        )
        self.input_words = (
